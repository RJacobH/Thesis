...
% So are Acknowledgments (proper English spelling)
	...
	\begin{figure}[h]
	\centering
	\includegraphics[clip=true, viewport=1in 5.5in 9in 11.3in, scale=0.5]{Images/Acknowledgements1}
	\caption{A; A $\rightarrow$ F[F+FA]-FB, B $\rightarrow$ F+F[–FA]+FB; $\theta$ = 32, $\phi$ = -7; Iterations = 7; IW = 4.5, WC = 0.85}
...
\chapter*{Abstract}
	...
	\caption{A; A $\rightarrow$ [F[–FA]FA]+A; $\theta$ = -120; Iterations = 5}
	\label {Acknowledgements1}
	\end{figure}
...
\chapter*{Dedication}
	...
	\caption{A; A $\rightarrow$ [F+F+FA]–F–FA; $\theta$ = 25; Iterations = 7}
	\label {Acknowledgements1}
	\end{figure}
	(I think I might rather this be one of the fractal things from ABOP, maybe one of the ones that use 'f')
...
%\textcolor{blue}{[Illustration of a simple L-System, perhaps just a fractal tree]}\\
	...
	\caption{A; A $\rightarrow$ F[+FA]–FA; $\theta$ = 32; Iterations = 5}
	\label {Introduction1}
	\end{figure}
...
%\textcolor{blue}{[Illustration of an L-System, something that parallels the first one, and yet does something dramatically different (maybe the same image, just upside down? Inverted somehow?) Maybe just another cool L-System.]}
	...
	\caption{A; A $\rightarrow$ +F[A+F–FA]–FA; $\theta$ = 60; Iterations = 5}
	\label {Introduction2}
	\end{figure}
...
%\textcolor{blue}{[Mathy L-System (Something Fractal)]}\\
	...
	\caption{A, A $\rightarrow$ [F[+F+FA]FA[F–FA]+FA]-FA, $\theta$ = 90, Iterations = 4}
	\label {Motivations1A}
	\end{figure}
	...
	\caption{A, A $\rightarrow$ [F[+F+FA]FA[F–FA]+FA]-FA, $\theta$ = 32, Iterations = 4}
	\label {Motivations1B}
	\end{figure}
	...
	\caption{A, A $\rightarrow$ +F[A]F–FA--FA, $\theta$ = -156, Iterations = 6}
	\label {Motivations2}
	\end{figure}
...
\section{The Path We Will Follow}
	...
	\caption{A, A $\rightarrow$ F[+F–A]F–F+A, B $\rightarrow$ F–FA, $\theta$ = 60, Iterations = 7}
	\label {Motivations3}
	\end{figure}
	...
	\caption{A, A $\rightarrow$ F[F+FA]–FA, $\theta$ = -120, Iterations = 12}
	\label {Motivations3}
	\end{figure}
	...
	\caption{A; A $\rightarrow$ B+A+B, B $\rightarrow$ A–B–A; $\theta$ = 60; Iterations = 6 (A and B draw)}
	\label {Sierpinski}
	\end{figure}
	...
	\caption{A; A $\rightarrow$ F[+A]f–A; $\theta$ = -90; Iterations = 7}
	\label {Sierpinski}
	\end{figure}
...
\section{Understanding through Definitions}
	...
	\caption{A; A $\rightarrow$ F+A; $\theta$ = -45; Iterations = 7}
	...
	\caption{A; A $\rightarrow$ F+B, B $\rightarrow$ F–A; $\theta$ = -122; Iterations = 7}
	\label {UnderstandingThroughDefinitions1}
	\end{figure}
...
This set of images is generated with the parameters, \code{start: A; rules: A $\rightarrow$ -FB, B $\rightarrow$ +FA; $\theta$ : 45$^{\circ}$}. Different iteration values are displayed in each image. Each new production starts from where the last one left off.\\
...
Here is a different set of images with the same start variable and angle, but different production rules. These rules are: \code{A $\rightarrow$ F-B, B $\rightarrow$ F+A}.\\
...
These sets of images show the progression of an L-System as the number of iterations increases. There is a sense that the L-System is ``growing" here, fittingly tied to their origin as plant-modeling software. Look at the following L-System and see how the different productions can be seen in the output. (The long straight sections are generated by \code{A} and the branch joints come from \code{B}.)
	...
	\caption{A; A $\rightarrow$ FFFB, B $\rightarrow$ +[– –FA]FA; $\theta$ = 60; Iterations = 11}
...
\label{Turtle-Graphics}
	...
	\begin{figure}[h]
	\centering
	\includegraphics[clip=true, viewport=0.7in 2in 8in 9in, scale=0.4]{Images/TurtleGraphics2}
	\caption{A simple star generated with Turtle}
	...
	\begin{figure}[h]
	\centering
	\includegraphics[clip=true, viewport=1.5in 1.3in 9.3in 8.7in, scale=0.4]{Images/TurtleGraphics3}
	\caption{Some recursive Turtle geometry}
